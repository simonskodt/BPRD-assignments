...
// The example forces all results of \n and numbers from 1 to 4.
// The first fromto dictates the number of lines while the second
// fromto dictates the number of numbers in each line.
let iconEx2 = Every(Write(And(FromTo(1,5), And(Write (CstS "\n"),FromTo(1,4)))));

let iconEx3 = Every(Write(Prim("*", FromTo(1,4), (And(Write (CstS "\n"),FromTo(1,4))))));

let str = "Hi there - if there are anyone"
let iconEx4 = (Every(Write(Find("there",str))));
let iconEx5 = (Every(Write(Find("e",str))));
let iconEx6 = (Every(Write(Prim("<", CstI 10, Find("e",str)))));
