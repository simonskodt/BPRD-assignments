...
class Machine {
  ...
  static void printStack(int N, int[] s, int bp, int sp) {
    ...
    while (bp != -999) {
      int[] arr = printStackFrame(s, bp, sp);
      bp = arr[0];
      sp = arr[1];
    }
    printGlobals(s, sp);
  ...
  static int[] printStackFrame(int[] s, int bp, int sp) {
    ...
    System.out.println(" s[" + sp + "]: bp = " + newBp);
    sp--;

    System.out.println(" s[" + sp + "]: ret = " + ret);
    sp--;

    return new int[] { newBp, sp-- };
  }

  static void printGlobals(int[] s, int sp) {
