...
let makelabenv (addr, labenv) instr = 
    ...
    | STOP           -> (addr+1, labenv)
    | PRINTCURFRM env  -> 
        // let len (_, s1:string) = 2 + s1.Length
        // 2 + sum of lengths of all strings in env + 2 for each pair
        (addr + (List.fold (fun acc (_, s1:string) -> acc + (2 + s1.Length) ) 2 env), labenv)
...
let rec emitints getlab instr ints = 
    ...
    | PRINTCURFRM env ->
      ...
      CODEPRINTCURFRM ::
      env.Length ::
      // (List.foldBack (fun elem acc -> codeVar elem acc) env ints)
