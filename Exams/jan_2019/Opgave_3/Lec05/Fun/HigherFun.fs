...
let rec eval (e : expr) (env : value env) : value =
    ...
    | Record e -> 
      ...
      let rec checkDup lst =
        ...
        | [] -> ()
        | [x] -> ()
        | (x, _)::lst -> if List.exists (fun (str, _) -> x = str) lst then failwith "eval Record: duplicate field name" else checkDup lst
      checkDup e

      RecordV (List.map (fun (name, e) -> (name, eval e env)) e) (* Exam *)
    | Field (e, name) ->
      match eval e env with
      | RecordV fields -> lookup fields name
...
let as01 = (Let("x", Record [("field1", CstI 32); ("field2", CstI 32)], Field (Var "x", "field1")));;
let as02 = (Let("x", Record [("field1", CstI 32); ("field1", CstI 32)], Field (Var "x", "field1")));;
