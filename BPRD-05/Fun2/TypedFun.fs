...
type tyexpr = 
  ...
  | ListExp of tyexpr list * typ (* Exercse 5.7 *)
...
type value = 
  ...
  | List of tyexpr list * typ (* Exercise 5.7 *)
  // | List of value list  (* maybe this? *)
...
let rec typ (e : tyexpr) (env : typ env) : typ =
    ...
    | CstB b -> TypB
    | Var x  -> lookup env x 
    | Prim(Item1 = ope; Item2 = e1; Item3 = e2;) -> 
      let t1 = typ e1 env
      let t2 = typ e2 env
    ...
    (* Exercise 5.7*)
    | ListExp(lst, t) -> 
      let typeCounter = List.fold (fun acc elem -> if ((typ elem env) <> t) then acc+1 else acc) 0 lst 
      if typeCounter = 0 then TypL t
      else failwith (sprintf "Type error: %A element(s) do not match list type." typeCounter)
